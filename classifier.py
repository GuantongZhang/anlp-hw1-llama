...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		dropped_out = self.dropout(last_hidden_state)  # dropout
		logits = self.classifier_head(dropped_out)  # pass through classifier head
		log_probs = F.log_softmax(logits, dim=-1)  # log-sofrmax probabilities
		return log_probs