...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        # todo
        attn_scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_dim)  # swap seqlen & head_dim
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        # todo
        norm_x = self.attention_norm(x)  # normalization (root mean square)
        attn_output = self.attention(norm_x)  # self-attention
        x = x + attn_output  # residual connection
        norm_x = self.ffn_norm(x)  # normalization
        ffn_output = self.feed_forward(norm_x)  # feed-forward network
        x = x + ffn_output
        return x
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_p=0.9):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                '''
                logits = logits / temperature  # scale logits with temperature
                probs = F.softmax(logits, dim=-1)  # normalization
                
                sorted_probs, sorted_indices = torch.sort(probs, descending=True, dim=-1)  # sort tokens
                cumulative_probs = torch.cumsum(sorted_probs, dim=-1)
                sorted_probs = torch.clamp(sorted_probs, min=1e-9, max=1.0)  # top-p cutoff, avoid NaN/inf
                sorted_probs = sorted_probs / sorted_probs.sum(dim=-1, keepdim=True)  # Normalize

                idx_next = torch.multinomial(sorted_probs, num_samples=1)  # sample
                idx_next = torch.gather(sorted_indices, -1, idx_next)  # Get actual token IDs
